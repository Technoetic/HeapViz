class TrackMapRenderer {
  ...
  _renderControls(container, svg) {
    ...
    if (h4) {
      ...
      // 이전 범례/버튼 제거 (종목 전환 시 중복 방지)
      h4.querySelectorAll('[data-track-ctrl]').forEach(el => el.remove());

      // 범례
      const legend = document.createElement('span');
      legend.setAttribute('data-track-ctrl', 'legend');
      ...
      btns.setAttribute('data-track-ctrl', 'buttons');
